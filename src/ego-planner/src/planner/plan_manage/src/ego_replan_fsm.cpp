...
namespace ego_planner
{
  ...
  void EGOReplanFSM::init(ros::NodeHandle &nh)
  {
    ...
    /* Subscribe via private ~odom_topic (set in launch); absolute names still work with private NodeHandle. */
  ...
  void EGOReplanFSM::waypointCallback(const nav_msgs::PathConstPtr &msg)
  {
    ...
    bool success = false;
    double gz = msg->poses[0].pose.position.z;
    if (std::fabs(gz) < 1e-3)
      gz = odom_pos_(2);
    end_pt_ << msg->poses[0].pose.position.x, msg->poses[0].pose.position.y, gz;
  ...
  void EGOReplanFSM::execFSMCallback(const ros::TimerEvent &e)
  {
    ...
    case EXEC_TRAJ:
    {
      ...
      if (t_cur > info->duration_ - 1e-2)
      {
        ...
        {
          // Trajectory time has run out but the vehicle is still far from goal.
          // Replan from current odometry to resynchronize execution and planning.
          changeFSMExecState(REPLAN_TRAJ, "FSM_ODOM_SYNC");
        }
        return;
      }
  ...
  bool EGOReplanFSM::planFromCurrentTraj()
  {
    ...
    Eigen::Vector3d traj_pt = info->position_traj_.evaluateDeBoorT(t_cur);
    Eigen::Vector3d traj_vel = info->velocity_traj_.evaluateDeBoorT(t_cur);
    Eigen::Vector3d traj_acc = info->acceleration_traj_.evaluateDeBoorT(t_cur);

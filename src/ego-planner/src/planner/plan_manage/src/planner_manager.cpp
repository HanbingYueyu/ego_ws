...
namespace ego_planner
{
  ...
  void EGOPlannerManager::initPlanModules(ros::NodeHandle &nh, PlanningVisualization::Ptr vis)
  {
    ...
    nh.param("manager/lock_ground_z", pp_.lock_ground_z_, false);
                                        ...
                                        Eigen::Vector3d local_target_vel, bool flag_polyInit, bool flag_randomPolyTraj)
  {
    ...
    const double ground_z = start_pt(2);
    if (pp_.lock_ground_z_)
    {
      start_pt(2) = ground_z;
      local_target_pt(2) = ground_z;
      start_vel(2) = 0.0;
      start_acc(2) = 0.0;
    ...
    do
    {
      ...
      else // Initial path generated from previous trajectory.
      {
        ...
        // Keep query time inside the valid trajectory span to avoid out-of-range
        // DeBoor evaluation when execution is lagging behind planning time.
        t_cur = std::max(0.0, std::min(local_data_.duration_ - 1e-3, t_cur));
        ...
        // Degenerate arc-length data can happen when we are very close to the
        // local target while reusing the previous trajectory. In that case,
        // switch to polynomial initialization instead of looping indefinitely.
        if (pseudo_arc_length.size() < 2 || pseudo_arc_length.back() < 1e-4)
        {
        ...
        do
        {
          ...
          if (++sample_retry > 10 && point_set.size() < 7)
          {
            flag_force_polynomial = true;
            flag_regenerate = true;
            break;
    ...
    if (pp_.lock_ground_z_)
      ctrl_pts.row(2).setConstant(ground_z);
    ...
    if (!pos.checkFeasibility(ratio, false))
    {
      ...
      {
        if (pp_.lock_ground_z_)
          optimal_control_points.row(2).setConstant(ground_z);
